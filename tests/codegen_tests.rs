...
mod codegen_tests {
    ...
    fn compile_and_run(program: &Program) -> Result<i64, CompileError> {
        ...
        let isa_builder =
            cranelift_native::builder().map_err(|e| CompileError::CraneliftError(e.to_string()))?;
        let isa = isa_builder
    ...
    fn test_nested_while_loops() {
        ...
        //     int i = 0;
        //     while (i < 3) {
        //         int j = 0;  // Reset j for each iteration of i
        //         while (j < 2) {
        //             sum = sum + (i * j);
        //             j = j + 1;
        //         }
                ...
                body: Some(AstBlock {
                    statements: vec![
                        ...
                        Stmt::While {
                            ...
                            body: Box::new(Stmt::Block(AstBlock {
                                statements: vec![
                                    // Initialize j = 0 inside outer loop
                                    Stmt::VarDecl(VarDecl {
                                        name: "j".to_string(),
                                        typ: AstType::Int64,
                                        initializer: Some(Expr::IntLiteral(0)),
        ...
        let result = compile_and_run(&program);
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), 3); // (0*0 + 0*1) + (1*0 + 1*1) + (2*0 + 2*1) = 0 + 1 + 2 = 3
    }
